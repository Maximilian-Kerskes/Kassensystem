...
function berechneGesamtbetrag() {
        return buchungen.reduce((summe, pos) => summe + pos.menge * pos.produkt.verkaufspreis, 0);
}
...
function renderBuchungen() {
        ...
        document.getElementById("gesamtbetrag").textContent = berechneGesamtbetrag().toFixed(2);
}
...
document.getElementById("abschicken").addEventListener("click", async () => {
        ...
        for (const pos of buchungen) {
                const response = await fetch(`${API_BASE_URL}/positionen`, {
                        ...
                        body: JSON.stringify({
                                produktnummer: pos.produkt.produktNr,
                                menge: pos.menge,
